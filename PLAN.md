...
```
[输入框上方]  左：目录 (git 分支)                        右：MCP 2/3 │ codex 周余 78% · 5d6h 后重置
[输入框]
...
| 其他状态 | `footerData.getExtensionStatuses()` 去掉 `mcp` | 按键名排序，去掉换行后用两个空格连接 |
...
- 输入框上方一行：先从开头省略目录，保留末尾的项目名与分支，例如 `…/piagent/e2e (main)`。左侧至少保留 16 列，分支名很长时至少保留「分支 + 10 列路径」；再窄就先省略额度的「后重置」三个字，再省略 MCP，最后只保留目录。
- 任何一行的可见宽度都不超过终端宽度。
